...
import { Contenido } from "./Componente";
import ApiService from "../../../../../data/services/ApiService";
import { orderBy } from "lodash";

export interface KReportePerformanceProps {
  MenuWeb: any;
}

export default function RptPerformance(props: KReportePerformanceProps): any {
  const [DataPagina, setDataPagina] = useState<any[]>([]);
  ...
  // #region Funciones de los filltros
  ...
  const CargarPaginas = async (dominio: any) => {
    ...
    await ApiService.getPaginasWithBudget(form).then((res: any) => {
      if (res.data.status === 200) {
        setDataPagina(orderBy(res.data.data, "sn_auditar", "desc"));
      } else {
  ...
  return (
    ...
    <KPage
      ...
      VisibleFiltros={true}
    >
      <Contenido DataTabla={DataPagina} />
    </KPage>
  );
}
